...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizeInBytes) {
	...
	vertexResourceDesc.Width = sizeInBytes;//リソースのサイズ。今回はVector4を3
	//バッファの場合はこれらは1にする決まり
...
{
	float cotan = 1 / tanf(fovY / 2);
	Matrix4x4 result;
	result.m[0][0] = 1 / aspectRaito * cotan;
	result.m[0][1] = 0;
	result.m[0][2] = 0;
	result.m[0][3] = 0;
	result.m[1][0] = 0;
...
D3D12_CPU_DESCRIPTOR_HANDLE GetCPUDescriptorHendle(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index) {
	D3D12_CPU_DESCRIPTOR_HANDLE handleCPU = descriptorHeap->GetCPUDescriptorHandleForHeapStart();
	handleCPU.ptr += (descriptorSize * index);
	return handleCPU;
}

D3D12_GPU_DESCRIPTOR_HANDLE GetGPUDescriptorHendle(ID3D12DescriptorHeap* descriptorHeap, uint32_t descriptorSize, uint32_t index) {
...
ID3D12Resource* CreateTextureResource(ID3D12Device* device, const DirectX::TexMetadata& metdata)
{
	...
	resourceDesc.MipLevels = UINT16(metdata.miscFlags);//mipmapの数
	resourceDesc.DepthOrArraySize = UINT16(metdata.arraySize);//奥行き or 配列Texture配列数
	resourceDesc.Format = metdata.format; //TextureのFormat
	resourceDesc.SampleDesc.Count = 1;//サンプリングカウント。1固定
	resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION(metdata.dimension);//Textureの次元数。普段使っているのは2次元
	...
	HRESULT hr = device->CreateCommittedResource(
		...
		nullptr,//Clera最適地。使わないのでnullptr
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height) {
	...
	resourceDesc.DepthOrArraySize = 1;//奥行き or 配列Texture配列数
...
void UploadTextureData(ID3D12Resource* texture, const DirectX::ScratchImage& mipImags)
{
	//Meta情報を取得
	const DirectX::TexMetadata& metadata = mipImags.GetMetadata();
	//全MipMapについて
	for (size_t mipLevel = 0; mipLevel < metadata.mipLevels; ++mipLevel) {
		//mioMapLevelを指定して各Imageを取得
		const DirectX::Image* img = mipImags.GetImage(mipLevel, 0, 0);
		//Textureに転送
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	HWND hwnd = CreateWindow(
		...
		L"LE2C_21_ハママツ_レイヤ",//タイトルバーの文字(何でも良い)
	...
	//ディスクリプタの先頭を取得する
	D3D12_CPU_DESCRIPTOR_HANDLE rtvStrtHandle = rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	...
	rootParameters[1].DescriptorTable.pDescriptorRanges = descriptorRangeForInstancing;//Tebleの中身の配列を使う
	rootParameters[1].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForInstancing);//Tebleで利用する数
	rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE; //DescriptorTableを使う
	rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL; //PixelShaderを使う
	rootParameters[2].DescriptorTable.pDescriptorRanges = descriptorRangeForInstancing; //Tableの中身の配列を設定
	...
	IDxcBlob* vertexShaderBlob =
		CompileShader(L"ParticleVS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(vertexShaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob =
		CompileShader(L"ParticlePS.hlsl", L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
...
#pragma region 頂点データ
	...
	*transformationMatrixDataSprite = MakeIdentity4x4();
	...
	//	float lat = -pi / 2.0f + kLatEvery * latIndex;//0
	//	//経度の方向に分割
...
#pragma region テクスチャ
	...
	//SRVを作成するDescriptorHeapの場所を決める
	D3D12_CPU_DESCRIPTOR_HANDLE textureSrvHandleCPU2 = GetCPUDescriptorHendle(srvDescriptorHeap,desriptorSizeSRV,2);
	D3D12_GPU_DESCRIPTOR_HANDLE textureSrvHandleGPU2 = GetGPUDescriptorHendle(srvDescriptorHeap,desriptorSizeSRV,2);
	//SRVの生成
	device->CreateShaderResourceView(textureResource2, &srvDesc2, textureSrvHandleCPU2);

#pragma endregion

#pragma region マテリアル(色)
...
#pragma region ワールド行列
	...
	// AL3でいうとworldTransform.matWorld
	...
	//単位行列に書き込んでおく
	*wvpData = MakeIdentity4x4(); // worldTransform.TranferMatrix();
...
#pragma endregion
	...
	D3D12_SHADER_RESOURCE_VIEW_DESC instancingSrvDese{};
	instancingSrvDese.Format = DXGI_FORMAT_UNKNOWN;
	instancingSrvDese.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	instancingSrvDese.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
	instancingSrvDese.Buffer.FirstElement = 0;
	instancingSrvDese.Buffer.NumElements = kNumInstance;
	instancingSrvDese.Buffer.StructureByteStride = sizeof(TransfomationMatrix);
	D3D12_CPU_DESCRIPTOR_HANDLE instancingSrvHandleCPU = GetCPUDescriptorHendle(srvDescriptorHeap, desriptorSizeSRV, 3);
	D3D12_GPU_DESCRIPTOR_HANDLE instancingSrvHandleGPU = GetGPUDescriptorHendle(srvDescriptorHeap, desriptorSizeSRV, 3);
	device->CreateShaderResourceView(instancingResource.Get(), &instancingSrvDese, instancingSrvHandleCPU);
...
#pragma region 更新処理
			...
			*transformationMatrixDataSprite = worldViewProjectionMatrixSprite;
			...
			ImGui::NewFrame();
			//開発用の処理。実際に開発用のUIを出す場合ここをゲーム固有の処理に置き換える
